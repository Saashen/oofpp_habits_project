...
from analytics import get_habits, get_period_habits, get_streak_for_habit, get_streaks_for_habits, get_weakest_habits
...
class TestHabit:
    ...
    def test_habit_instance(self, capsys):
        habit = DatabaseHabit(
            'test_title5', 'test_description5', 'weekly', date(2023, 8, 5))
        habit.store(self.db)
        captured = capsys.readouterr()
        assert captured.out == 'The habit with the title `test_title5` was successfully added.\n'

        habit.complete_task(self.db, date(2023, 8, 12))
        captured = capsys.readouterr()
        assert captured.out == 'The successful streak count was updated.\nYou have a new record! Your longest streak for the habit `test_title5` is 1.\n'
        ...
        captured = capsys.readouterr()
        assert captured.out == 'You broke your habit! You skipped more than 7 day(s)\nThe successful streak count was updated.\n'

        habit.delete(self.db)
        captured = capsys.readouterr()
        assert captured.out == 'The completed tasks of the habit `test_title5` were successfully deleted.\nThe habit with the title `test_title5` was successfully deleted.\n'

    def test_analytics(self, capsys):
        get_habits(self.db)
        captured = capsys.readouterr()
        assert captured.out == 'Habit titles are: \n1. test_title1 \n2. test_title2 \n3. test_title3 \n4. test_title4 \n'

        get_period_habits(self.db, 'daily')
        captured = capsys.readouterr()
        assert captured.out == 'The habits of the period `daily` are: \n1. test_title1 \n2. test_title3 \n'
        ...
        captured = capsys.readouterr()
        assert captured.out == 'The longest streaks of all habits are: \ndaily\n1. test_title3 - 3 \n2. test_title1 - 2 \nweekly\n1. test_title4 - 4 \n2. test_title2 - 2 \n'

        get_weakest_habits(self.db)
        captured = capsys.readouterr()
        assert captured.out == 'Lately you struggled the most with these habits: \ndaily\ntest_title1 - 2 \nweekly\ntest_title2 - 1 \n'
